...
#### **TC4: Username đã tồn tại - hiển thị error**
```java
testSignup_UsernameExists_ShowsError()
```
- **Mục đích**: Kiểm tra server-side validation khi username đã tồn tại trong database
- **Given**: Username đã tồn tại trong DB (`tai`), email mới hợp lệ
- **When**: Điền form với username đã tồn tại và submit
- **Then**: 
  - Hiển thị error message "❌ Username đã được sử dụng"
  - Form không được submit thành công
  - Vẫn ở trang signup

#### **TC5: Email đã tồn tại - hiển thị error**
```java
testSignup_EmailExists_ShowsError()
```
- **Mục đích**: Kiểm tra server-side validation khi email đã tồn tại trong database
- **Given**: Email đã tồn tại trong DB (`anhtai80605@gmail.com`), username mới
- **When**: Điền form với email đã tồn tại và submit
- **Then**: 
  - Hiển thị error message "❌ Email đã được sử dụng"
  - Form không được submit thành công
  - Vẫn ở trang signup

#### **TC6: Username và Email đều đã tồn tại - ưu tiên username**
```java
testSignup_BothUsernameAndEmailExist_ShowsUsernameError()
```
- **Mục đích**: Kiểm tra logic ưu tiên khi cả username và email đều đã tồn tại
- **Given**: Cả username (`tai`) và email (`anhtai80605@gmail.com`) đều đã tồn tại trong DB
- **When**: Điền form với cả username và email đã tồn tại và submit
- **Then**: 
  - Hiển thị error message "❌ Username đã được sử dụng" (ưu tiên username)
  - Form không được submit thành công
  - Vẫn ở trang signup

### Coverage:
- ✅ Client-side email validation (JavaScript)
- ✅ Server-side validation (username/email existence)
- ✅ UI/UX feedback (error messages, alerts)
- ✅ Form submission behavior
- ✅ Success message display
- ✅ Error priority handling (username trước email)

### Page Objects:
- `SignupPage` - Page Object cho signup page với các methods:
  - `navigateToSignup()` - Navigate đến signup page và disable HTML5 validation
  - `signup(username, email, password)` - Điền form và submit
  - `isSuccessMessageDisplayed()` - Kiểm tra success message
  - `isErrorMessageDisplayed()` - Kiểm tra error message (hỗ trợ nhiều locators)
  - `getErrorMessage()` - Lấy error message từ page (hỗ trợ alert và page elements)
  - `hasAlertWithError()` - Kiểm tra alert popup
  - `disableHTML5Validation()` - Disable HTML5 validation để test các trường hợp invalid
...
### **4. Test Coverage**
- **AuthControllerSeleniumTest**: 6 test cases - bao phủ UI validation, server-side validation, và user experience
...
|-----------|---------------|----------------|----------|
| **AuthControllerSeleniumTest** | 6 | Đăng ký tài khoản UI/UX | Client-side validation, Server-side validation, User experience |
...
**Tổng cộng: 20 test cases** với coverage cao cho UI/UX của các features quan trọng.

### Coverage đạt được:
- ✅ **Feature 1 (AuthController):** 6 test cases - Coverage ~90% (UI/UX + Server-side validation)
...
├── selenium/
│   ├── AuthControllerSeleniumTest.java          (6 tests)
...
2. Database phải có dữ liệu test:
   - Products (cho CartController tests)
   - Clubs (cho AdminController tests)
   - User đã tồn tại: `username: tai`, `email: anhtai80605@gmail.com` (cho AuthController tests)
